...
class EarlyCroP(EarlyTrain):
    ...
    def finish_run(self):
        ...
        for i, (input, targets) in enumerate(batch_loader):
            ...
            grad_w_p = torch.autograd.grad(loss, outputs, create_graph=False)[0]
            outputs = self.model(input.to(self.args.device))
            loss = self.criterion(torch.nn.functional.softmax(outputs.requires_grad_(True), dim=1),
                                  targets.to(self.args.device)).sum()
            grad_f = torch.autograd.grad(loss, outputs, create_graph=True)[0]
            z = (grad_w_p * grad_f).sum()
            with torch.no_grad():
                bias_parameters_grads = torch.autograd.grad(z, outputs)[0]
