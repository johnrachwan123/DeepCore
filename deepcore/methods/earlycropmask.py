...
class EarlyCroPMask(EarlyTrain):
    ...
    def finish_run(self):
        ...
        for i, (input, targets) in enumerate(batch_loader):
            ...
            self.model.mask = torch.nn.Parameter(torch.ones([input.shape[0]]), requires_grad=True)
            input = (input.permute(3, 1, 2, 0).cuda() * self.model.mask.cuda()).permute(3, 1, 2, 0)
            ...
            grad_w_p = torch.autograd.grad(loss, self.model.mask, create_graph=False)[0]
            self.model.mask = torch.nn.Parameter(torch.ones([input.shape[0]]), requires_grad=True)
            input = (input.permute(3, 1, 2, 0).cuda() * self.model.mask.cuda()).permute(3, 1, 2, 0)
            outputs = self.model(input.to(self.args.device))
            loss = self.criterion(torch.nn.functional.softmax(outputs.requires_grad_(True), dim=1),targets.to(self.args.device)).sum()
            grad_f = torch.autograd.grad(loss, self.model.mask, create_graph=True)[0]
            z = (grad_w_p * grad_f).sum()

            with torch.no_grad():
                bias_parameters_grads = torch.abs(torch.autograd.grad(z, self.model.mask)[0])
                self.norm_matrix[i * self.args.selection_batch:min((i + 1) * self.args.selection_batch, sample_num), self.cur_repeat] = bias_parameters_grads
                # self.norm_matrix[i * self.args.selection_batch:min((i + 1) * self.args.selection_batch, sample_num), self.cur_repeat] = torch.norm
                    # torch.norm(torch.cat([bias_parameters_grads, (
